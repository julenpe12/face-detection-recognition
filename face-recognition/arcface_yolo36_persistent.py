...
DATABASE_FILE = "trained/arcface_db.pkl"
...
class FaceRecognizerArcFaceTorch:
    ...
    def __getstate__(self):
        ...
        state["model"] = None
        state["model_path"] = state.get("model_path", "models/arcface.pt")
        return state
...
def save_database(recognizer, path=DATABASE_FILE):
    os.makedirs(os.path.dirname(path), exist_ok=True)
    data = {"features": recognizer.features, "labels": recognizer.labels}
    with open(path, "wb") as f:
        pickle.dump(data, f)
    print("Database saved.")

def load_database(recognizer, path=DATABASE_FILE):
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if training_mode and detections:
            ...
            cv2.putText(
                frame,
                "{}/{}".format(sample_count, target_samples),
                (10, 30),
            ...
            if sample_count >= target_samples:
                ...
                training_mode = False
                sample_count = 0
                print("Saved '{}'".format(current_label))
        else:
