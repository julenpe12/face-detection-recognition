########################################
# Script 1: Sin persistencia (no guardado de datos)
########################################
import cv2
import numpy as np
...
def main():
    ...
    print("Presiona 'T' para iniciar entrenamiento, 'q' para salir.")
    ...
    while True:
        ...
        if training_mode and len(faces) > 0:
            ...
            recognizer.add_training_sample(roi, label)
            count += 1
            cv2.putText(frame, f"Training {label}: {count}/{target_samples}",
                        (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2)
            cv2.rectangle(frame, (x, y), (x+w, y+h), (255, 0, 0), 2)
            if count >= target_samples:
                training_mode = False
                count = 0
                print(f"Entrenamiento completado para '{label}'.")

        else:
            if not recognizer.features_database:
                cv2.putText(frame, "No hay datos de entrenamiento",
                            (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
            for (x, y, w, h) in faces:
                roi = frame[y:y+h, x:x+w]
                lbl, dist = recognizer.recognize(roi)
                color = (0, 255, 0) if dist < recognizer.recognition_threshold else (0, 0, 255)
                cv2.putText(frame, f"{lbl} ({dist:.2f})", (x, y-10), cv2.FONT_HERSHEY_SIMPLEX, 0.8, color, 2)
        ...
        if key in (ord('T'), ord('t')) and not training_mode:
            label = input("Etiqueta para entrenamiento: ").strip()
            if label:
                training_mode = True
                count = 0
                print(f"Modo entrenamiento: '{label}'")
