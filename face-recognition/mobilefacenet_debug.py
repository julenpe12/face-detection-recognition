...
class FaceDetectorYuNet:
    ...
    def detect(self, image):
        ...
        h, w = image.shape[:2]
        # YuNet requires the current frame size each call
...
class FaceRecognizerMobileFaceNetTorch:
    ...
    def __getstate__(self):
        ...
        state["model"] = None  # TorchScript is not picklable
        return state
...
def main():
    ...
    while True:
        ...
        # -------------------------
        now = time.time()
        fps_buf.append(1.0 / (now - prev_frame_time))
        prev_frame_time = now
        avg_fps = sum(fps_buf) / len(fps_buf)

        cpu_buf.append(process.cpu_percent(interval=None) / logical_cores)
            ...
            if len(faces) > 0:
                ...
                if 0 <= x < frame.shape[1] and 0 <= y < frame.shape[0]:
                    ...
                    if face_roi.size:
                        ...
                        cv2.putText(
                            frame,
                            f"Training {current_label}: {training_sample_count}/{target_training_samples}",
                            (10, 30),
        ...
        # Overlay metrics
        # -------------------------
        cv2.putText(frame, f"FPS: {int(avg_fps)}", (7, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (100, 255, 0), 2, cv2.LINE_AA)
        cv2.putText(frame, f"CPU: {avg_cpu:.1f}% per core", (7, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (100, 255, 0), 2, cv2.LINE_AA)
        cv2.putText(frame, f"Mem: {avg_mem:.2f} MB", (7, 120), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (100, 255, 0), 2, cv2.LINE_AA)
